...
const bindSwipe = (element: HTMLElement, handlers: SwipeHandlers, types: string[] = ['mouse', 'touch']) => {
  ...
  element.addEventListener(getEventName(types, 'start'), onStart);
  element.addEventListener(getEventName(types, 'move'), onMove);
  element.addEventListener(getEventName(types, 'end'), onEnd);
  if (types.includes('touch')) {
    element.addEventListener(getEventName(types, 'cancel'), onCancel);
  }

  return () => {
    element.removeEventListener(getEventName(types, 'start'), onStart);
    element.removeEventListener(getEventName(types, 'move'), onMove);
    element.removeEventListener(getEventName(types, 'end'), onEnd);
    if (types.includes('touch')) {
      element.removeEventListener(getEventName(types, 'cancel'), onCancel);
    }
  };
};
