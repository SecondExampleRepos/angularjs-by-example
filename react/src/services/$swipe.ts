// react/src/services/$swipe.ts

interface Point {
  x: number;
  y: number;
}

interface SwipeHandlers {
  start?: (point: Point, event: Event) => void;
  move?: (point: Point, event: Event) => void;
  end?: (point: Point, event: Event) => void;
  cancel?: (event: Event) => void;
}

const mouseEvents = {
  start: "mousedown",
  move: "mousemove",
  end: "mouseup"
};

const touchEvents = {
  start: "touchstart",
  move: "touchmove",
  end: "touchend",
  cancel: "touchcancel"
};

const getPoint = (event: any): Point => {
  const e = event.touches && event.touches.length ? event.touches[0] : event;
  return { x: e.clientX, y: e.clientY };
};

const getEventNames = (types: string[], eventName: string): string => {
  return types.map(type => (type === "mouse" ? mouseEvents[eventName] : touchEvents[eventName])).join(" ");
};

const bindSwipe = (element: HTMLElement, handlers: SwipeHandlers, types: string[] = ["mouse", "touch"]) => {
  let startPoint: Point | null = null;
  let isSwiping = false;
  let deltaX = 0;
  let deltaY = 0;

  const handleStart = (event: Event) => {
    startPoint = getPoint(event);
    isSwiping = true;
    deltaX = 0;
    deltaY = 0;
    handlers.start && handlers.start(startPoint, event);
  };

  const handleMove = (event: Event) => {
    if (!isSwiping || !startPoint) return;
    const point = getPoint(event);
    deltaX += Math.abs(point.x - startPoint.x);
    deltaY += Math.abs(point.y - startPoint.y);
    startPoint = point;
    if (deltaX > 10 || deltaY > 10) {
      if (deltaY > deltaX) {
        isSwiping = false;
        handlers.cancel && handlers.cancel(event);
      } else {
        event.preventDefault();
        handlers.move && handlers.move(point, event);
      }
    }
  };

  const handleEnd = (event: Event) => {
    if (!isSwiping) return;
    isSwiping = false;
    handlers.end && handlers.end(getPoint(event), event);
  };

  const handleCancel = (event: Event) => {
    isSwiping = false;
    handlers.cancel && handlers.cancel(event);
  };

  element.addEventListener(getEventNames(types, "start"), handleStart);
  element.addEventListener(getEventNames(types, "move"), handleMove);
  element.addEventListener(getEventNames(types, "end"), handleEnd);
  if (types.includes("touch")) {
    element.addEventListener(touchEvents.cancel, handleCancel);
  }

  return () => {
    element.removeEventListener(getEventNames(types, "start"), handleStart);
    element.removeEventListener(getEventNames(types, "move"), handleMove);
    element.removeEventListener(getEventNames(types, "end"), handleEnd);
    if (types.includes("touch")) {
      element.removeEventListener(touchEvents.cancel, handleCancel);
    }
  };
};

export { bindSwipe, Point, SwipeHandlers };