...
class Resource {
  ...
  public createResource(actions: ResourceActions = {}): any {
    const finalActions = { ...this.defaults.actions, ...actions };

    const resource: any = {};

    Object.keys(finalActions).forEach((actionName) => {
      const action = finalActions[actionName];
      ...
      resource[actionName] = (params: any = {}, data: any = {}, success?: (value: any) => void, error?: (reason: any) => void) => {
        ...
        const instance: ResourceInstance = isPostOrPutOrPatch ? data : {};
