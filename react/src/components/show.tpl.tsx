...
const ShowFrame: React.FC<ShowFrameProps> = ({ genres, show }) => {
    const [defaultImage] = useState('assets/images/loading.jpg');
    const [fallbackImage] = useState('assets/images/fallback.jpg');
    const imageUrl = `http://image.tmdb.org/t/p/w780/${show.backdrop_path}`;

    const getBackgroundColor = (index: number) => {
        return `rgba(59, 185, 187, ${genres.length / index / 5})`;
    };
        ...
        <div className="show-frame">
            <ul className="genres">
                {genres.map((genre, index) => (
                    <li key={index} className="animate-repeat" style={{ backgroundColor: getBackgroundColor(index + 1) }}>
                        {genre.name}
                    </li>
                ))}
            </ul>
