...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch tutorials or set initial state here
    fetch('/api/tutorials')
      ...
      <div className="home-banner">
        <div className="container inner">
          ...
          <div className="home-buttons">
            <a href="https://github.com/RevillWeb/angularjs-by-example" target="_blank" className="btn btn-lg btn-primary" rel="noopener noreferrer"><span className="icon icon-github2"></span> GitHub</a>
            <a href="http://www.revillweb.com" target="_blank" className="btn btn-lg btn-default" rel="noopener noreferrer"><span className="icon icon-home"></span> RevillWeb</a>
          </div>
        </div>
      </div>
        ...
        <ul className="tutorials">
          {tutorials.map((tutorial, index) => (
            <li key={index} className={tutorial.link === '#' ? 'offline' : ''}>
              ...
              <a href={tutorial.link} className={`btn btn-lg btn-primary ${tutorial.link === '#' ? 'disabled' : ''}`}><span className="icon icon-arrow-up-right5"></span> View</a>
              <div className="clearfix"></div>
            </li>
          ))}
        </ul>
        {tutorials.length === 0 && <p className="no-data tuts">More tutorials coming soon...</p>}
      </div>
    </div>
  );
};
