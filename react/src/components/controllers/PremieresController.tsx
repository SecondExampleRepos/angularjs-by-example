// Converted from src/sections/premieres/premieres.ctrl.js

import React from 'react';
import Show from '../../directives/Show'; // Corrected the import to match the file name casing
import { Helmet } from 'react-helmet'; // Importing react-helmet for setting page title and description

interface PremieresControllerProps {
  shows: Array<any>;
}

const PremieresController: React.FC<PremieresControllerProps> = ({ shows }) => {
  return (
    <div>
      <Helmet>
        <title>PREMIERES</title>
        <meta name="description" content="Brand new shows showing this month." />
      </Helmet>
      <h1>Premieres</h1>
      <p>Brand new shows showing this month.</p>
      <div>
        {shows.map((show, index) => (
          <div key={index}>
            {/* Assuming Show is a component that takes a show prop */}
            <Show show={show} />
          </div>
        ))}
      </div>
    </div>
  );
};

export default PremieresController;
