...
// Assuming PageValues is a service that needs to be replaced with React state
const usePageValues = () => {
    ...
    useEffect(() => {
        setTitle("HOME");
        setDescription("Learn AngularJS using best practice real world examples.");
    }, []);
...
const HomeController: React.FC = () => {
    const { title, description } = usePageValues();
            ...
            <ul>
                {tutorials.map((tutorial, index) => (
                    <li key={index}>
                        ...
                        <a href={tutorial.link}>Read more</a>
                    </li>
                ))}
            </ul>
        </div>
