...
type InterceptorFunction = (promise: Promise<IHttpResponse<any>>) => Promise<IHttpResponse<any>>;
...
function configs($httpProvider: IHttpProvider): void {
    const interceptor = function($location: ILocationService, $log: ILogService, $q: IQService): InterceptorFunction {
        function error(response: IHttpResponse<any>): Promise<any> {
        ...
        return function(promise: Promise<IHttpResponse<any>>): Promise<IHttpResponse<any>> {
...
function runs($rootScope: IRootScopeService, PageValues: PageValuesType): void {
    $rootScope.$on('$routeChangeStart', function(): void {
        PageValues.loading = true;
    });
    $rootScope.$on('$routeChangeSuccess', function(): void {
